...
def mindestbestand_vorschlaege(lager, fenster=VERBRAUCHSFENSTER_TAGE, puffer_tage=14,
    ...
    for a in lager.artikel.values():
        ...
        vorschlaege.append({
            ...
            # Ohne gemessenen Verkauf reicht eine Schwelle von 1: der Artikel
            # meldet sich dann erst, wenn er wirklich leer ist, statt dauerhaft
            # in der Warnliste zu stehen.
...
def monatsbericht_html(lager, zahlen, logo_cid="logo.png"):
    """Monatsauswertung als E-Mail im Erscheinungsbild der Marke."""
    from . import marke
    from .umsatz import euro, zahl

    farben = marke.palette(lager)
    primaer, akzent, hell = farben["primaer"], farben["akzent"], farben["hell"]
    d, vor = zahlen, zahlen["vormonat"]
    ...
    def entwicklung(jetzt, vorher):
        ...
        anteil = (jetzt - vorher) / vorher * 100
        if abs(anteil) < 0.5:
            farbe, text = "9CA3AF", "unverändert zum Vormonat"
        else:
            farbe = "059669" if anteil > 0 else "DC2626"
            text = f"{anteil:+.0f} % zum Vormonat"
        return (f'<div style="font:600 11px/1.4 Helvetica,Arial,sans-serif;color:#{farbe};'
                f'padding-top:3px;">{text}</div>')

    def kachel(wert, beschriftung, farbe, zusatz=""):
        return (
            f'<td align="center" width="50%" style="padding:18px 8px;background:#{hell};'
            f'border-radius:6px;">'
            f'<div style="font:700 28px/1.1 Helvetica,Arial,sans-serif;color:#{farbe};">{wert}</div>'
            f'<div style="font:400 11px/1.4 Helvetica,Arial,sans-serif;color:#6B7280;'
            f'text-transform:uppercase;letter-spacing:.06em;padding-top:5px;">{beschriftung}</div>'
            f'{zusatz}</td>'
        )

    def kennzahl(beschriftung, wert):
        return (f'<tr><td style="padding:4px 0;font:400 13px/1.5 Helvetica,Arial,sans-serif;'
                f'color:#6B7280;">{beschriftung}</td>'
                f'<td align="right" style="padding:4px 0;font:600 13px/1.5 '
                f'Helvetica,Arial,sans-serif;color:#{primaer};">{wert}</td></tr>')

    def liste(titel, eintraege, mit_anteil=False):
        if not eintraege:
            return ""
        gesamt = d["umsatz"] or 1
        zeilen_html = []
        for nummer, (name, anzahl, betrag) in enumerate(eintraege):
            grund = hell if nummer % 2 else "FFFFFF"
            anteil = (f'<td align="right" style="padding:7px 10px;background:#{grund};'
                      f'font:400 12px/1.4 Helvetica,Arial,sans-serif;color:#9CA3AF;">'
                      f'{zahl(betrag / gesamt * 100, 0)}&nbsp;%</td>') if mit_anteil else ""
            zeilen_html.append(
                f'<tr><td style="padding:7px 10px;background:#{grund};'
                f'font:400 13px/1.4 Helvetica,Arial,sans-serif;color:#374151;">{name}</td>'
                f'<td align="right" style="padding:7px 10px;background:#{grund};'
                f'font:600 13px/1.4 Helvetica,Arial,sans-serif;color:#{primaer};">{anzahl}</td>'
                f'<td align="right" style="padding:7px 10px;background:#{grund};'
                f'font:400 13px/1.4 Helvetica,Arial,sans-serif;color:#6B7280;">'
                f'{euro(betrag)}&nbsp;€</td>{anteil}</tr>'
            )
        return (
            f'<tr><td style="padding:20px 28px 4px;font:700 11px/1.6 Helvetica,Arial,sans-serif;'
            f'color:#{primaer};text-transform:uppercase;letter-spacing:.07em;">{titel}</td></tr>'
            f'<tr><td style="padding:4px 28px 0;"><table width="100%" cellpadding="0" '
            f'cellspacing="0" role="presentation" style="border-collapse:collapse;">'
            f'{"".join(zeilen_html)}</table></td></tr>'
    ...
    kennzahlen = [kennzahl("Vormonat " + d["vormonat_name"],
                           f"{euro(vor['umsatz'])} € · {vor['verkaeufe']} Verkäufe")]
    if d["verkaeufe"]:
        kennzahlen.append(kennzahl("Durchschnitt je Tag",
                                   f"{euro(d['umsatz'] / d['tage_im_monat'])} € · "
                                   f"{zahl(d['verkaeufe'] / d['tage_im_monat'])} Verkäufe"))
        kennzahlen.append(kennzahl("Durchschnitt je Verkauf",
                                   f"{euro(d['umsatz'] / d['verkaeufe'])} €"))
    if d["bester_tag"]:
        tag, anzahl, betrag = d["bester_tag"]
        kennzahlen.append(kennzahl("Bester Tag",
                                   f"{tag.strftime('%d.%m.')} · {euro(betrag)} € · {anzahl} Verkäufe"))
    kennzahlen.append(kennzahl("Verkaufstage",
                               f"{d['tage_mit_verkauf']} von {d['tage_im_monat']}"))
    ...
    return f"""<!doctype html>
<html lang="de"><body style="margin:0;padding:24px 12px;background:#F4F4F5;">
<table align="center" width="620" cellpadding="0" cellspacing="0" role="presentation"
  ...
  <tr><td style="padding:24px 28px 18px;">
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr>
      <td align="left" valign="middle">{logo_block}</td>
      <td align="right" valign="middle">
        <div style="font:700 18px/1.2 Helvetica,Arial,sans-serif;color:#{primaer};">Monatsauswertung</div>
        <div style="font:400 12px/1.5 Helvetica,Arial,sans-serif;color:#6B7280;">
          {d['monat_name']} {d['jahr']}</div>
      </td>
    </tr></table>
  </td></tr>
  ...
  <tr><td style="padding:22px 28px 6px;">
    <table width="100%" cellpadding="0" cellspacing="8" role="presentation"><tr>
      {kachel(f"{euro(d['umsatz'])} €", "Umsatz", primaer,
              entwicklung(d['umsatz'], vor['umsatz']) + pfandzeile)}
      {kachel(d['verkaeufe'], "Verkäufe", akzent,
              entwicklung(d['verkaeufe'], vor['verkaeufe']))}
    </tr></table>
  </td></tr>
  ...
  {liste("Nach Standort", d["je_standort"], mit_anteil=True)}
  {liste("Meistverkauft", d["je_produkt"][:15])}
  {stoerungstext}

  <tr><td style="padding:18px 28px 24px;font:400 12px/1.6 Helvetica,Arial,sans-serif;color:#6B7280;">
    Alle {len(d["je_produkt"])} verkauften Artikel liegen als CSV-Datei im Anhang.
  </td></tr>
